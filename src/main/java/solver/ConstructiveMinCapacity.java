...
public class ConstructiveMinCapacity implements Solver {
  private OptimizationProblem problem_;
  private Solution solution_;

  public ConstructiveMinCapacity(OptimizationProblem problem) {
    problem_ = problem;
  }
  ...
  void assignSuppliers(List<Integer> suppliers_sorted) throws Exception {
    ...
    while (!suppliers_sorted.isEmpty()) {
      ...
      int min_capacity_in_door = getMinFittingElement(in_capacities, pallets);
  ...
  private void assignCustomers(List<Integer> customers_sorted) throws Exception {
    ...
    while (!customers_sorted.isEmpty()) {
      ...
      int min_capacity_out_door = getMinFittingElement(out_capacities, pallets);
  ...
  private int getMinFittingElement(int[] array, int threshold) {
    int min = Integer.MAX_VALUE; // Max int
    int absolute_min = 0;
    ...
    return min == Integer.MAX_VALUE ? absolute_min : min;
  }
}
